  ...
  return (
    <div className="home-container">
      ...
      <section className="hero">
        <h1>
          {text?.title || '스킬 교환 플랫폼'}
          <span className="hero-underline"></span>
        </h1>

        <p className="hero-description">
          {(text?.heroDescription || '새로운 사람들과 스킬을 교환하며\n함께 성장해보세요.\n언어, 디자인, 프로그래밍, 음악 등\n다양한 스킬을 자유롭게 교환할 수 있습니다.').split('\n').map((line, index) => (
            <span key={index}>
              {line}
              <br />
            </span>
      ...
      <section className="skills-section">
        <h2 className="section-title">{text?.popularSkills || '인기 스킬'}</h2>

        <div className="skills-grid">
          {skills.map((skill, index) => (
            <div className="skill-card" key={index}>
              <h3>{skill}</h3>
              <p>{text?.popularSkillExchange || '인기 스킬 교환'}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="ai-section">
        <h2 className="section-title">{text?.aiSmartMatch || 'AI 스마트 매칭 시스템'}</h2>

        <p className="ai-desc">
          {(text?.aiFeatureDescription || '스킬 태그, 관심 분야,\n학습 목표, 활동 시간 등을 기반으로\n가장 잘 맞는 교환 파트너를\nAI가 추천합니다.').split('\n').map((line, index) => (
            <span key={index}>
              {line}
              <br />
            </span>
      ...
      <footer className="footer">
        {text?.footerText || '© 2026 Skill Exchange Platform'}
      </footer>
    </div>
  )
}
